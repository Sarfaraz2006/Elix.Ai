...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val modelAdapter = object : ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, modelLabels) {
            override fun getView(pos: Int, cv: View?, parent: ViewGroup): View {
                val v = super.getView(pos, cv, parent) as TextView
                v.setTextColor(Color.WHITE); v.textSize = 13f; v.setPadding(8,12,8,12); return v
            }
            override fun getDropDownView(pos: Int, cv: View?, parent: ViewGroup): View {
                val v = super.getDropDownView(pos, cv, parent) as TextView
                v.setTextColor(Color.WHITE); v.setBackgroundColor(Color.parseColor("#1A1A1A"))
                v.setPadding(16,16,16,16); v.textSize = 13f; return v
            }
        }
        binding.modelSpinner.adapter = modelAdapter
        binding.modelSpinner.setSelection(models.indexOfFirst {
            it.first == prefs.getString("gemini_model", models[0].first)
        }.coerceAtLeast(0))

        val voiceAdapter = object : ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, voices) {
            override fun getView(pos: Int, cv: View?, parent: ViewGroup): View {
                val v = super.getView(pos, cv, parent) as TextView
                v.setTextColor(Color.WHITE); v.textSize = 13f; v.setPadding(8,12,8,12); return v
            }
        ...
        binding.voiceSpinner.setSelection(voices.indexOf(
            prefs.getString("gemini_voice","Aoede")).coerceAtLeast(0))
        ...
        primeAdapter = PrimeContactAdapter(primeContacts) { pos ->
            primeContacts.removeAt(pos); primeAdapter.notifyItemRemoved(pos)
        }
        ...
        binding.saveBtn.setOnClickListener {
            ...
            editor.putString("user_name", binding.nameInput.text.toString().trim().ifEmpty { "friend" })
            ...
            editor.putString("personality_mode", when(binding.radioGroup.checkedRadioButtonId) {
    ...
    private fun showAddPrimeDialog() {
        ...
        AlertDialog.Builder(this)
            .setTitle("Add Prime Contact").setView(view)
